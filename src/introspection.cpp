...
Message IntrospectableAdaptor::Introspect(const CallMessage &call) {
    LOG("requested introspection data");
    ...
    for (iti = _interfaces.begin(); iti != _interfaces.end(); ++iti) {
        LOG("introspecting interface %s", iti->first.c_str());
        ...
        if (intro) {
            ...
            for (const IntrospectedProperty *p = intro->properties; p->name; ++p) {
                ...
                xml << "\n\t\t<property name=\"" << p->name << "\"" << " type=\"" << p->type << "\"" << " access=\""
                    << access << "\"/>";
            }
            ...
            for (const IntrospectedMethod *m = intro->methods; m->args; ++m) {
                ...
                for (const IntrospectedArgument *a = m->args; a->type; ++a) {
                    xml << "\n\t\t\t<arg direction=\"" << (a->in ? "in" : "out") << "\"" << " type=\"" << a->type
                        << "\"";
...
IntrospectedInterface *IntrospectableAdaptor::introspect() const {
    static IntrospectedArgument Introspect_args[] = { { "data", "s", false }, { 0, 0, 0 } };
    static IntrospectedMethod Introspectable_methods[] = { { "Introspect", Introspect_args }, { 0, 0 } };
    static IntrospectedMethod Introspectable_signals[] = { { 0, 0 } };
    static IntrospectedProperty Introspectable_properties[] = { { 0, 0, 0, 0 } };
    static IntrospectedInterface Introspectable_interface = { introspectable_name, Introspectable_methods,
        Introspectable_signals, Introspectable_properties };
