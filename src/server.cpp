...
void Server::Private::on_new_conn_cb(DBusServer *server, DBusConnection *conn, void *data) {
    ...
    LOG("incoming connection 0x%08x", conn);
}
...
Server::Server(const char *address) {
    ...
    LOG("server 0x%08x listening on %s", server, address);
...
Dispatcher *Server::setup(Dispatcher *dispatcher) {
    LOG("registering stubs for server %p", _pvt->server);

    Dispatcher *prev = _pvt->dispatcher;

    dbus_server_set_watch_functions(_pvt->server, Dispatcher::Private::on_add_watch, Dispatcher::Private::on_rem_watch,
                                    Dispatcher::Private::on_toggle_watch, dispatcher, 0);

    dbus_server_set_timeout_functions(_pvt->server, Dispatcher::Private::on_add_timeout,
                                      Dispatcher::Private::on_rem_timeout, Dispatcher::Private::on_toggle_timeout,
                                      dispatcher, 0);
